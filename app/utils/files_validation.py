...
def validate_size(file_bytes: bytes, max_mb: int = MAX_FILE_SIZE):
    size_mb = len(file_bytes)/(1024*1024)
    if size_mb > max_mb:
        raise HTTPException(status_code=400, detail=f"File size {size_mb:.2f}MB exceeds limit of {max_mb}MB.")
